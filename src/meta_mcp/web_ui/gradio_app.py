...
class GradioWebInterface:
        ...
        with gr.Blocks(
            ...
            with gr.Row():
                with gr.Column(scale=3):
                    ...
                    gr.Markdown("*Interactive interface for testing tool retrieval and managing configuration*")
    ...
    def _create_tool_testing_tab(self):
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                with gr.Group():
                    ...
                    with gr.Row():
                        gr.Button("📁 File Operations", size="sm", variant="secondary").click(
                            lambda: "search for files and directories",
                            outputs=[query_input]
                        )
                        gr.Button("🔧 Development", size="sm", variant="secondary").click(
                ...
                with gr.Group():
                    ...
                    with gr.Row():
                        ...
                        server_filter = gr.Dropdown(
                            ...
                            choices=["all"] + [s.name for s in self.config.child_servers],
                            value="all",
        ...
        test_button.click(
            ...
            inputs=[
                query_input, strategy_dropdown, max_tools_slider,
                threshold_slider, server_filter
            ],
            outputs=[
                results_json, status_indicator, selected_tool, tools_df,
                confidence_display, execution_time_display, tools_found_display
            ],
            show_progress="full"
        )
        ...
        clear_button.click(
            fn=lambda: ("", {}, [], [], 0.0, 0.0, 0, '<div class="metric-card">Ready to test tool selection</div>'),
            outputs=[
                query_input, results_json, selected_tool, tools_df,
                confidence_display, execution_time_display, tools_found_display, status_indicator
            ]
        )
        ...
        execute_button.click(
            ...
            outputs=[execution_result, execution_status, execution_history, history_display],
            show_progress="full"
        )
    ...
    def _create_config_tab(self):
        ...
        with gr.Row():
            with gr.Column(scale=2):
                ...
                with gr.Row():
                    ...
                    reload_button = gr.Button("🔄 Reload from File", variant="secondary")
            ...
            with gr.Column(scale=1):
                ...
                config_sections = gr.JSON(
                    label="Current Config Structure",
                    value=self._get_config_summary()
                )
        ...
        restore_button.click(
            fn=self._restore_backup,
            outputs=[config_editor, backup_status]
        )
  ...
  "mcpServers": {
    ...
    "git": {
      ...
      "env": {}
        ...
        import_button.click(
            ...
            inputs=[json_input, merge_with_existing, auto_generate_descriptions, validate_commands],
            outputs=[import_status, import_stats, imported_tools_df],
            show_progress="full"
        )
        
        validate_json_button.click(
            fn=self._validate_mcp_json,
            inputs=[json_input],
            outputs=[import_status]
        )
        
        clear_json_button.click(
            fn=lambda: ("", '<div class="metric-card">Ready to import tools</div>', {}, []),
    ...
    def _test_tool_selection_with_progress(
        self, query: str, strategy: str, max_tools: int, threshold: float, server_filter: str
        ...
        async def _async_test_with_progress():
            if not query.strip():
                return {}, '<div class="metric-card error-text">❌ Please enter a query to test</div>', [], [], 0.0, 0.0, 0
            ...
            if server_filter != "all":
                available_tools = [t for t in available_tools if t.server_name == server_filter]
            ...
            tool_choices = [f"{tool.name} ({tool.server_name})" for tool in result.tools[:max_tools]]
            
            # All tools dataframe
            tools_data = [
                [tool.name, tool.server_name, tool.description[:50] + "...", tool.usage_count]
                for tool in available_tools
            ]
            
            return (
                results_dict, status_html, tool_choices, tools_data,
                result.confidence_score, execution_time, len(result.tools)
            )
        ...
        try:
            ...
            error_status = f'<div class="metric-card error-text">❌ Error: {str(e)}</div>'
    ...
    def _test_tool_selection(
        ...
        try:
            results_dict, execution_info, tool_choices, tools_data = asyncio.run(_async_test())
            return results_dict, execution_info, gr.update(choices=tool_choices), tools_data
        except Exception as e:
            self.logger.error(f"Tool selection test failed: {e}")
    ...
    def _validate_json(self, json_text: str) -> str:
        ...
        try:
            ...
            return f'<div class="metric-card error-text">❌ Invalid JSON: {str(e)}</div>'
    ...
    def _reset_config(self) -> tuple[str, str]:
        ...
        try:
            ...
            config_yaml = yaml.dump(default_config.model_dump(), default_flow_style=False)
    ...
    def _restore_backup(self) -> tuple[str, str]:
        ...
        try:
            ...
            config_yaml = yaml.dump(self.config_backup["config"], default_flow_style=False)
            return config_yaml, f"✅ Restored from backup ({self.config_backup['timestamp']})"
        except Exception as e:
    ...
    def _refresh_status(self) -> tuple[str, dict, list, dict]:
        ...
        async def _async_refresh():
            ...
            if hasattr(self.server_instance, 'child_manager'):
                child_status = await self.server_instance.child_manager.get_server_status()
    ...
    def _get_recent_logs(self, log_level: str = "INFO") -> str:
        ...
        try:
            ...
            with open(log_file, 'r') as f:
                ...
                if log_level != "ALL":
                    filtered_lines = [line for line in recent_lines if log_level in line]
    ...
    def _load_json_from_file(self, file_path) -> tuple[str, str]:
        ...
        try:
            ...
            return "", f'<div class="metric-card error-text">❌ Error reading file: {str(e)}</div>'
                ...
                try:
                    ...
                    if validate_commands:
                        ...
                        if not shutil.which(command):
                            import_stats["warnings"].append(f"Command '{command}' not found in PATH for server '{server_name}'")
                    ...
                    import_stats["errors"].append(f"Error processing server '{server_name}': {str(e)}")
                    continue
    ...
    def _generate_tools_for_server(self, server_name: str, command: str, args: list, auto_generate_descriptions: bool) -> list[dict]:
        ...
        if detected_type:
            ...
            tools = [
                ...
                {"name": f"{server_name}_execute", "description": "Execute server command"}
            ]
        ...
        for tool in tools:
            ...
            if auto_generate_descriptions and detected_type:
                tool["description"] = f"{tool['description']} (via {server_name} server)"
    ...
    def _merge_imported_tools(self, imported_tools: list) -> None:
        ...
        try:
            ...
            if hasattr(self.server_instance, 'available_tools'):
                ...
                existing_ids = {tool.id for tool in self.server_instance.available_tools}
                unique_new_tools = [tool for tool in new_tools if tool.id not in existing_ids]
