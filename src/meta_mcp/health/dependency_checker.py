...
class DependencyChecker:
    ...
    async def check_qdrant_collections(self, host: str, port: int, collections: list[str]) -> dict[str, bool]:
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    existing_collections = {
                        collection["name"] for collection in data.get("result", {}).get("collections", [])
                    }
    ...
    async def verify_docker_health(self, service_names: list[str]) -> dict[str, dict[str, Any]]:
